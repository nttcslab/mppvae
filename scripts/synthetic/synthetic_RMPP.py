...
import time
from util import *
from model_RMPP import *

ratnames = ['Lorenz_3D', 'Lorenz_2D']
ratname= ratnames[name_ind]
...
data = np.load(os.path.join(ROOTDIR, 'SPIKE_data', ratname, 'data' + str(perm_ind) + '.npy'), allow_pickle=True)[()]
...
dirname = os.path.join(ROOTDIR, 'results', 'RMPP', ratname, 'batch' + str(perm_ind))
...
batch_dts_padded_train = tf.concat([batch_ts_padded_train[:, :1] - batch_ttildes_padded_train[:, :1], (batch_ts_padded_train[:, 1:] - batch_ts_padded_train[:, :-1]) * tf.reduce_sum(batch_masks_train, axis=2)[: , 1:]], axis=1)
batch_dts_padded_val = tf.concat([batch_ts_padded_val[:, :1] - batch_ttildes_padded_val[:, :1], (batch_ts_padded_val[:, 1:] - batch_ts_padded_val[:, :-1]) * tf.reduce_sum(batch_masks_val, axis=2)[: , 1:]], axis=1)
batch_dts_padded_test = tf.concat([batch_ts_padded_test[:, :1] - batch_ttildes_padded_test[:, :1], (batch_ts_padded_test[:, 1:] - batch_ts_padded_test[:, :-1]) * tf.reduce_sum(batch_masks_test, axis=2)[: , 1:]], axis=1)

batch_kappas_padded_train = tf.concat([batch_dts_padded_train[:, :, :, tf.newaxis], batch_kappas_padded_train], axis=-1)
batch_kappas_padded_val = tf.concat([batch_dts_padded_val[:, :, :, tf.newaxis], batch_kappas_padded_val], axis=-1)
batch_kappas_padded_test = tf.concat([batch_dts_padded_test[:, :, :, tf.newaxis], batch_kappas_padded_test], axis=-1)

n_params.update({'n_recog': [50, 50, 50, 50], 'n_scale': [50, 50, 50, 50], 'n_gener_kappa': [50, 50, 50, 50],
                 'n_recog_y': [50, 50, 50, 50], 'n_gener_y': [50, 50, 50, 50], 'n_recog_kappa': [50, 50, 50, 50],
                 'n_z': 10, 'n_x': 10, 'n_mixture': 10})

n_params.update({'n_batch': n_batch})
n_params.update({'dropout_rate': dropout_rate})
...
for epoch in tqdm(range(training_epochs)):
    ...
    if (epoch + 1) % eval_step == 0:
        ...
        print('Log x likelihood val:', '{:.3f}'.format(log_x_likelihood_val))
