...
sns.set()
sns.set_style('white')
colors_dict = {'CVAE': 'purple', 'GMM': 'blue', 'RMPP': 'green', 'GRU': 'gold', 'DNN': 'orange', 'WF': 'magenta'}
...
for ratname in ratnames:
    ...
    for method in methods:
        ...
        for perm_ind in range(10):

            log_likelihood = np.load(os.path.join(ROOTDIR, 'results', method, ratname, 'batch' + str(perm_ind), 'log_likelihood.npy'))
        ...
        if method != 'RMPP':
            ...
            for perm_ind in range(10):
                ...
                sep, ttilde, y, y_smoothed_means, y_smoothed_covs = np.load(os.path.join(ROOTDIR, 'results', method, ratname, 'batch' + str(perm_ind),  'reconstruct_test.npy'), allow_pickle=True)
...
fig.savefig(os.path.join(ROOTDIR, 'figures', 'synthetic_nll.png'), bbox_inches='tight')
...
fig.savefig(os.path.join(ROOTDIR, 'figures', 'synthetic_mse.png'), bbox_inches='tight')
