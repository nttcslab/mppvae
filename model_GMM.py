...
class GMM(tf.keras.Model):
    ...
    def __init__(self, n_params):
        ...
        self.gener_x_mean, self.gener_x_prec_values = tf.Variable(tf.random.normal(shape=(n_mixture, n_site, n_x))), tf.Variable(tf.random.normal(shape=(n_mixture, n_site, n_x*(n_x+1)//2)))
        self.gener_kappa_mean, self.gener_kappa_prec_values = tf.Variable(tf.random.normal(shape=(n_mixture, n_site, n_kappa_max))), tf.Variable(tf.random.normal(shape=(n_mixture, n_site, n_kappa_max)))
        self.gener_log_weights = tf.Variable(tf.zeros((n_mixture, n_site)))
        ...
        self.lambda_variables = [self.gener_x_mean, self.gener_x_prec_values, self.gener_kappa_mean, self.gener_kappa_prec_values, self.gener_log_weights, self.log_lambda]
        ...
        self.lambda_optimizer = tf.keras.optimizers.Adam(self.learning_rate)
        self.eta_optimizer_3 = tf.keras.optimizers.Adam(self.learning_rate)
        with tf.device('/cpu:0'):
            self.eta_optimizer_2 = tf.keras.optimizers.Adam(self.learning_rate)
        self.eta_optimizer_1 = tf.keras.optimizers.Adam(self.learning_rate)
...
def _apply_lambda_gradients(model, kappas, masks, x, n_monte):
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        lambda_weights = tf.stop_gradient(tf.exp(tf.math.log_softmax(lambda_loss, axis=1)))
        lambda_all = tf.reduce_sum(tf.math.multiply_no_nan(log_lambda + lambda_loss, lambda_weights), axis=1)

        lambda_g_weights = tf.stop_gradient(tf.exp(log_lambda + tf.reduce_logsumexp(lambda_g_loss, axis=1, keepdims=True) + tf.math.log_softmax(lambda_g_loss, axis=1)))
        lambda_g_all = tf.reduce_sum(tf.math.multiply_no_nan(log_lambda + lambda_g_loss, lambda_g_weights), axis=1)

        lambda_cost = - tf.reduce_sum(lambda_all * tf.reduce_sum(masks, axis=1)) + tf.reduce_sum(lambda_g_all)

    lambda_gradients = tape.gradient(lambda_cost, model.lambda_variables)

    vec = - tf.linalg.matvec(x_prec, matmul_mask_tranpose_vec(masks, x)[:, tf.newaxis] - x_mean)
    x_grad = tf.reduce_sum(tf.math.multiply_no_nan(vec, lambda_weights[:, :, :, tf.newaxis]), axis=1)

    vec_g = - tf.linalg.matvec(x_g_prec, x[:, tf.newaxis] - x_g_mean)
    x_g_grad = tf.reduce_sum(tf.math.multiply_no_nan(vec_g, lambda_g_weights[:, :, :, tf.newaxis]), axis=1)

    x_grad = tf.reduce_sum(- matmul_mask_vec(masks, x_grad) + x_g_grad, axis=1, keepdims=True)

    if not tf.math.reduce_any([tf.math.reduce_any([tf.math.is_nan(grad), tf.math.is_inf(grad)]) for grad in lambda_gradients if grad is not None]):
        model.lambda_optimizer.apply_gradients(zip(lambda_gradients, model.lambda_variables))

    return lambda_cost, lambda_cost, x_grad
...
def _compute_lambda_loss(model, x, kappas, n_monte):
    ...
    x_mean, x_prec_values = model.gener_x_mean, model.gener_x_prec_values
    x_prec_diag, x_prec_tri = x_prec_values[:, :, :n_x], fill_triangular(x_prec_values[:, :, n_x:])
    kappa_mean, kappa_prec_values = model.gener_kappa_mean, model.gener_kappa_prec_values
    kappa_prec_diag = kappa_prec_values[:, :, :n_kappa_max]
    ...
    latent_loss = tf.math.log_softmax(model.gener_log_weights, axis=0)
...
compute_eta_loss_1 = tf.function(_compute_eta_loss_1, experimental_compile=True, reduce_retracing=True)
compute_eta_loss_2 = tf.function(_compute_eta_loss_2, experimental_compile=True, reduce_retracing=True)
compute_eta_loss_3 = tf.function(_compute_eta_loss_3, experimental_compile=True, reduce_retracing=True)

compute_lambda_loss = tf.function(_compute_lambda_loss, experimental_compile=True, reduce_retracing=True)
compute_lambda_g_loss = tf.function(_compute_lambda_g_loss, experimental_compile=True, reduce_retracing=True)
...
def _eval_log_likelihood(model, kappas, masks, y, log_scale_train, log_scale_eval, n_monte):
    ...
    lambda_scale_all = log_scale_train + tf.reduce_logsumexp(log_lambda + lambda_loss, axis=1)
    lambda_g_scale_all = tf.exp(log_scale_train + tf.reduce_logsumexp(log_lambda + lambda_g_loss, axis=1) - log_scale_eval)
...
def _eval_log_lambda_derivative(model, x, kappas, log_scale_train, n_monte):
    ...
    weights = tf.math.softmax(lambda_loss, axis=1)
    ...
    hessian_log_lambda_x = - weighted_vec[:, :, tf.newaxis] * weighted_vec[:, :, :, tf.newaxis]
...
def _eval_lambda_g_derivative(model, x, log_scale_train, n_monte):
    ...
    weights = tf.math.softmax(lambda_g_loss, axis=1)
    scale = tf.exp(log_scale_train + log_lambda + tf.reduce_logsumexp(lambda_g_loss, axis=1))
    weighted_vec = tf.reduce_sum(weights[:, :, :, tf.newaxis] * vec, axis=1)

    grad_lambda_x = scale[:, :, tf.newaxis] * weighted_vec
    hessian_lambda_x = scale[:, :, tf.newaxis, tf.newaxis] * (weighted_vec[:, :, tf.newaxis] * weighted_vec[:, :, :, tf.newaxis] + tf.reduce_sum((vec - weighted_vec[:, tf.newaxis])[:, :, :, tf.newaxis] * (vec - weighted_vec[:, tf.newaxis])[:, :, :, :, tf.newaxis] * weights[:, :, :, tf.newaxis, tf.newaxis], axis=1))
...
def  _assign_log_lambda(model, batch_kappas, batch_masks, batch_ys, n_monte):
    ...
    def _lambda_g_cost_batch(state, elem):
        ...
        lambda_g_cost_epoch = tf.reduce_logsumexp(tfp.math.reduce_logmeanexp(lambda_g_loss, axis=1), axis=0)
